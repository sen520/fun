...
class Scene:
    ...
    def strarray(self):
        var = [
            ...
            "<feGaussianBlur in=\"SourceAlpha\" stdDevivation=\"5\"></feGaussianBlur>\n",
...
class Text:
    ...
    def strarray(self):
        if (self.isItbig == True):
            ...
            if (self.isBold == True):
                retval = ["<text y= \"%d\" x=\"%d\" style=\"font-weight:bold;fill:%s\">%s</text>\n" % (
                    self.y, self.x, self.color, self.txt)]
            else:
                retval = [
                    "<text y=\"%d\" x=\"%d\" style=\"fill:%s\">%s</text>\n" % (self.y, self.x, self.color, self.txt)]
        return retval
...
class Circle:
    ...
    def strarray(self):
        ...
        retval = [
            ...
            "<clipPath id=\"dataseg-%d\"><rect height=\"%d\" width=\"%d\" y=\"%d\" x=\"%d\"></rect>" % (
                svcount, diam, diam, fillamt, xpos),
            "</clipPath>\n",
        ]
        svcount += 1
        return retval
